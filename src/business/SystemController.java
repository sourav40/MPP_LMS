...
//            System.out.println(rec[1]);
    ...
    public  String[] isBookOverDue(String isbn) {
    	...
    	List<CheckoutRecordEntry> checkoutEntries= member.getValue().getCheckoutRecord().getCheckoutRecordEntries();
    	
    	for(CheckoutRecordEntry ce : checkoutEntries) {
    		if(today.isAfter((convertToLocalDateViaInstant(ce.getDueDate().getTime()))) && !ce.getBookCopy().isAvailable() ) {
    			//all overdue books
